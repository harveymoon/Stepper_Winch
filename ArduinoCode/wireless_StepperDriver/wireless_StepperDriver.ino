...
const char* DeviceName = "stepper_winch_01";
...
AccelStepper stepper(1, D3, D2); // mode, step, dir
...
void routeCapture(OSCMessage &msg, int addrOffset) {
  doCapture();
}

void routeMicroSteps(OSCMessage &msg, int addrOffset ) {
  int one = msg.getInt(0);
  int two = msg.getInt(1);
  int three = msg.getInt(2);
  Serial.println("Incomming MicroSetting : ");
  Serial.print(one);
  Serial.print(' ');
  Serial.print(two);
  Serial.print(' ');
  ...
  if (one == 1) {
    digitalWrite(ms1, HIGH); // setPin
  } else {
    digitalWrite(ms1, LOW); // setPin
  }
  delay(100);
  if (two == 1) {
    digitalWrite(ms2, HIGH); // setPin
  } else {
    digitalWrite(ms2, LOW); // setPin
  }
  delay(100);
  if (three == 1) {
    digitalWrite(ms3, HIGH); // setPin
  } else {
    digitalWrite(ms3, LOW); // setPin
  }
  delay(100);
...
void loop() {
  ...
  if ( (size = Udp.parsePacket()) > 0)
  {
    ...
    if (!msgIN.hasError()) {
      ...
      msgIN.route("/microstep", routeMicroSteps);
